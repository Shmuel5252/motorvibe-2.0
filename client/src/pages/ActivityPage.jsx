    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center px-0 sm:px-4">
            <button
                ...
                aria-label="סגור"
            />
            <div className={`relative z-10 w-full sm:max-w-lg max-h-[92dvh] overflow-y-auto custom-scrollbar rounded-t-3xl sm:rounded-3xl border border-white/10 shadow-2xl ${className}`}>
                {children}
            </div>
        </div>
    );
...
function HistoryRideMap({ points, isMapLoaded, mapLoadError }) {
    ...
    if (mapLoadError)
        return <div className="flex h-full items-center justify-center text-xs text-slate-400">שגיאה בטעינת המפה</div>;
    if (!isMapLoaded)
        return <div className="flex h-full items-center justify-center text-xs text-slate-400">טוען מפה...</div>;
    ...
    return (
        <GoogleMap
            ...
            options={{ styles: DARK_MAP_STYLE, disableDefaultUI: true, clickableIcons: false, gestureHandling: "cooperative" }}
        >
            {points.length >= 2 && (
                <PolylineF path={points} options={{ strokeColor: "#34d399", strokeOpacity: 0.9, strokeWeight: 4 }} />
            )}
            {points.length >= 2 && <MarkerF position={points[0]} label="A" title="התחלה" />}
            {points.length >= 2 && <MarkerF position={points[points.length - 1]} label="B" title="סיום" />}
        </GoogleMap>
    );
}
...
function RouteDetailMap({ route, isMapLoaded, mapLoadError, isValidMapPoint, getSafePolylinePath, getRoutePolylinePath }) {
    const [mapInstance, setMapInstance] = useState(null);
    ...
    const origin = isValidMapPoint(route?.origin || route?.fromLatLng) ? (route?.origin || route?.fromLatLng) : null;
    const destination = isValidMapPoint(route?.destination || route?.toLatLng) ? (route?.destination || route?.toLatLng) : null;

    useEffect(() => {
        if (!isMapLoaded || !window.google || !mapInstance || !origin || !destination) {
            return;
        }
        try {
    ...
    useEffect(() => {
        if (!isMapLoaded || !window.google || !mapInstance || safePath.length < 2) return;
        try {
            const bounds = new window.google.maps.LatLngBounds();
    ...
    if (!isMapLoaded)
        return <div className="flex h-full items-center justify-center text-sm text-slate-400">טוען מפה...</div>;
    if (mapLoadError)
        return <div className="flex h-full items-center justify-center text-sm text-slate-400">שגיאה בטעינת המפה</div>;
...
function HistoryDetailModal({
    ride, onClose, apiClient, fetchHistoryFromServer, fetchRoutesFromServer,
    ...
    if (path.length >= 2) {
        ...
        points = [{ lat: snap.start.lat, lng: snap.start.lng }, { lat: snap.end.lat, lng: snap.end.lng }];
    }
    ...
    const handleConvert = async () => {
        ...
        if (snap?.start && snap?.end) {
            payload = { title: (raw.name || raw.title || snap.title || "מסלול חדש").trim(), start: snap.start, end: snap.end };
        } else if (path.length >= 2) {
            const first = path[0], last = path[path.length - 1];
            payload = { title: (raw.name || raw.title || "רכיבה חופשית").trim(), start: { lat: first.lat, lng: first.lng, label: "נקודת התחלה" }, end: { lat: last.lat, lng: last.lng, label: "יעד" } };
        } else {
        ...
        <ModalBackdrop onClose={onClose} className="bg-slate-950/80 backdrop-blur-2xl border border-white/10 shadow-2xl">
            <div className="p-5 space-y-5">
                ...
                <div className="flex items-start justify-between gap-3">
                    <div>
                        <p className="text-xl font-bold text-white drop-shadow-md">{ride.title}</p>
                        <p className="mt-1 text-xs text-slate-400 font-medium">{ride.date}</p>
                    </div>
                ...
                {localImageUrl ? (
                    <div className="relative overflow-hidden rounded-2xl border border-white/10 shadow-lg group">
                        <img src={imgSrc(localImageUrl)} alt="תמונת רכיבה" className="h-48 w-full object-cover" />
                        <label className="absolute right-3 top-3 flex cursor-pointer items-center gap-1.5 rounded-full bg-black/60 px-3 py-1.5 text-xs font-semibold text-white shadow-sm backdrop-blur-md transition hover:bg-black/80">
                            {isUploading ? <span>מעלה...</span> : <><Camera className="w-3.5 h-3.5 text-slate-300" /><span>שנה תמונה</span></>}
                    ...
                    <div className="flex h-32 flex-col items-center justify-center rounded-2xl border border-dashed border-white/20 bg-white/5 backdrop-blur-sm">
                        <label className="flex cursor-pointer flex-col items-center gap-2 transition hover:opacity-80">
                            <span className="flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-300 backdrop-blur-md shadow-sm border border-emerald-400/20">
                                {isUploading ? "מעלה..." : <><Plus className="w-4 h-4" />הוסף תמונה לרכיבה</>}
                            </span>
                            <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} disabled={isUploading} />
                        </label>
                ...
                <div className="grid grid-cols-3 gap-3">
                    {[["משך", ride.duration], ["מרחק", ride.distance], ["תאריך", ride.date]].map(([label, val]) => (
                ...
                {mapApiKey?.trim() && points.length >= 2 && (
                    <div className="overflow-hidden rounded-xl border border-white/5 bg-white/5 backdrop-blur-md shadow-sm">
                        <p className="border-b border-white/5 px-3 py-2 text-xs font-semibold text-slate-300">מפת מסלול</p>
                        <div className="h-[200px]">
                            <HistoryRideMap points={points} isMapLoaded={isMapLoaded} mapLoadError={mapLoadError} />
                        </div>
                    </div>
                )}
                ...
                {modalError && <p className="text-xs text-rose-400 text-center">{modalError}</p>}
                ...
                {convertError && <p className="text-xs text-rose-400 text-center mt-1">{convertError}</p>}
                {convertSuccess && <p className="text-xs text-emerald-400 text-center mt-1">נשמר כמסלול ✅</p>}
            </div>
        </ModalBackdrop>
    );
}
...
function RouteDetailModal({
    ...
    apiClient, authToken, fetchRoutesFromServer,
}) {
    const [deleteError, setDeleteError] = useState("");

    const routeStyle = route?.routeType || route?.style || route?.routeStyle || "עירוני";
            ...
            <div className="p-6 space-y-6">
                ...
                {route?.imageUrl && (
                    <div className="overflow-hidden rounded-2xl border border-white/10 shadow-lg">
                        <img src={imgSrc(route.imageUrl)} alt="תמונת מסלול" className="h-48 w-full object-cover" />
                    </div>
                )}
...
function AddRouteModal({
    ...
    const isPlacesApiReady =
        isMapLoaded && !mapLoadError && typeof window !== "undefined" && Boolean(window.google?.maps?.places);

    const resolveSuggestionToPoint = (suggestion) =>
        new Promise((resolve) => {
            if (!window.google?.maps?.places) { resolve(null); return; }
            const placeId = suggestion?.place_id;
            if (!placeId) { resolve(null); return; }
            ...
            const onGeoFallback = () => {
                ...
                new window.google.maps.Geocoder().geocode({ placeId }, (results, status) => {
                    if (status === "OK" && results?.[0]?.geometry?.location) {
                        ...
                        resolve({ point: { lat: l.lat(), lng: l.lng() }, label: fallbackLabel });
                    } else { resolve(null); }
                });
            };
            try {
                const svc = new window.google.maps.places.PlacesService(document.createElement("div"));
                svc.getDetails({ placeId, fields: ["geometry", "name", "formatted_address"] }, (place, status) => {
                    if (status === window.google.maps.places.PlacesServiceStatus.OK && place?.geometry?.location) {
                        const l = place.geometry.location;
                        resolve({ point: { lat: l.lat(), lng: l.lng() }, label: place.formatted_address || place.name || fallbackLabel });
                    } else { onGeoFallback(); }
                });
            } catch { onGeoFallback(); }
        });
    ...
    const applySuggestion = async (target, suggestion) => {
        ...
        if (target === "from") { setNewOriginLabel(label); setOriginSuggestions([]); }
        else { setNewDestinationLabel(label); setDestinationSuggestions([]); }
        setActiveSuggestionField(null);
        const resolved = await resolveSuggestionToPoint(suggestion);
        if (!resolved?.point) return;
        if (target === "from") { setNewOriginLatLng(resolved.point); setNewOriginLabel(label || resolved.label); }
        else { setNewDestinationLatLng(resolved.point); setNewDestinationLabel(label || resolved.label); }
    ...
    const openMapPicker = (target) => {
        ...
        const labelText = (target === "from" ? newOriginLabel : newDestinationLabel).trim();
        const nextId = mapPickRequestIdRef.current + 1;
        mapPickRequestIdRef.current = nextId;
        if (isValidMapPoint(existing)) { setMapPickCenter(existing); setMapPickStatus(""); return; }
        setMapPickCenter(ISRAEL_DEFAULT_CENTER);
        if (!labelText || !isMapLoaded || mapLoadError || !window.google?.maps?.Geocoder) { setMapPickStatus(""); return; }
        setMapPickStatus("מחפש מיקום...");
        new window.google.maps.Geocoder().geocode({ address: labelText }, (results, status) => {
    ...
    const closeMapPicker = () => { mapPickRequestIdRef.current += 1; setMapPickStatus(""); setActiveMapPickField(null); };

    const handleSubmit = async () => {
        const title = newRouteTitle.trim();
        const from = newOriginLabel.trim();
        const to = newDestinationLabel.trim();
        if (!title || !from || !to) { setNewRouteLocationError("יש למלא שם, מוצא ויעד"); return; }
        if (!authToken) { setNewRouteLocationError("נדרש אימות משתמש לשמירה בשרת"); return; }
        if (!newOriginLatLng || !newDestinationLatLng) { setNewRouteLocationError("לשמירה בשרת יש לבחור מוצא ויעד במפה"); return; }
        setNewRouteLocationError("");
        try {
            await apiClient.post("/routes", {
                title,
                start: { lat: newOriginLatLng.lat, lng: newOriginLatLng.lng, label: from },
                end: { lat: newDestinationLatLng.lat, lng: newDestinationLatLng.lng, label: to },
                routeType: newRouteType,
                difficulty: newRouteIsTwisty ? getAdjustedDifficultyForTwisty(newRouteDifficulty) : newRouteDifficulty,
                isTwisty: newRouteIsTwisty,
            }, { headers: { Authorization: `Bearer ${authToken}` } });
            await fetchRoutesFromServer(authToken);
            /* clear form */
            setNewRouteTitle(""); setNewOriginLabel(""); setNewOriginLatLng(null);
            setNewDestinationLabel(""); setNewDestinationLatLng(null);
            setNewRouteType("עירוני"); setNewRouteDifficulty("בינוני"); setNewRouteIsTwisty(false);
            setOriginSuggestions([]); setDestinationSuggestions([]); setActiveSuggestionField(null);
            closeMapPicker();
            onClose();
        } catch (error) {
            if (error?.response?.status === 401) { handleUnauthorized(); return; }
                            ...
                            <div className="h-64 overflow-hidden rounded-xl border border-white/10 shadow-inner relative">
                                ...
                                <GoogleMap
                                    ...
                                    onClick={(event) => {
                                        ...
                                        const point = { lat, lng };
                                        if (activeMapPickField === "from") { setNewOriginLatLng(point); setNewOriginLabel("נקודה נבחרה"); }
...
export default function ActivityPage({
    ...
    setIsRideActive, setIsRidePaused, setIsRideMinimized,
    setDidStartFromRoute, onNavigate,
    /* map */
    mapApiKey, isMapLoaded, mapLoadError,
    /* routes */
    ...
    getAdjustedDifficultyForTwisty,
    getRoutePolylinePath, isValidMapPoint, getSafePolylinePath,
    fetchRoutesFromServer, authToken, apiClient,
    /* new-route form */
    newRouteTitle, setNewRouteTitle,
    newOriginLabel, setNewOriginLabel,
    newOriginLatLng, setNewOriginLatLng,
    ...
    /* history */
    historyRides,
    historyFilters, selectedHistoryFilter, setSelectedHistoryFilter,
    ...
    return (
        <div className="relative min-h-screen bg-transparent" dir="rtl">
            ...
            {isRideActive && !isRideMinimized && (
                <div className="mx-4 mt-4 flex items-center justify-between rounded-2xl border border-emerald-500/30 bg-emerald-900/20 px-4 py-3">
                    ...
                    <button type="button" onClick={() => onNavigate("ride")} className="text-xs text-emerald-300 underline underline-offset-2">חזור לרכיבה</button>
                </div>
            )}
      ...
      ══════════════════════════════════════════════ */}
            <div className="flex-1 px-4 relative z-10 pt-6">
                ...
                <h1 className="text-3xl font-bold text-white mb-5 drop-shadow-md">פעילות רכיבה</h1>
                ...
                <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl shadow-2xl">
                    <div className="flex items-center justify-around text-center">
                        <div className="flex flex-col items-center">
                            ...
                            <p className="text-3xl font-black leading-none text-white">{totalRoutes}</p>
                            <p className="mt-1.5 text-[11px] font-medium tracking-wide text-slate-400">מסלולים</p>
                        </div>
                        <div className="h-10 w-px bg-white/10" />
                        <div className="flex flex-col items-center">
                            <span className="mb-1.5 text-xl opacity-80">📏</span>
                            <p className="text-3xl font-black leading-none text-white">{parseFloat(totalKm.toFixed(0))}</p>
                            <p className="mt-1.5 text-[11px] font-medium tracking-wide text-slate-400">סה״כ ק״מ</p>
                        </div>
                        <div className="h-10 w-px bg-white/10" />
                        <div className="flex flex-col items-center">
                            <Bike className="w-6 h-6 mb-2 text-slate-400 opacity-80" />
                            <p className="text-3xl font-black leading-none text-white">{historyRides.length}</p>
                            <p className="mt-1.5 text-[11px] font-medium tracking-wide text-slate-400">רכיבות</p>
                        </div>
                    </div>
                </div>
      ...
      ══════════════════════════════════════════════ */}
            <div className="relative -mt-8 rounded-t-3xl bg-transparent border-t border-white/10 min-h-[60vh] pb-32">
                    ...
                    <div className="px-4 mt-6 space-y-3">
                        ...
                        {visibleRides.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-16 text-center opacity-80">
                                <div className="mb-4 flex justify-center"><Bike className="w-12 h-12 text-slate-500 opacity-50" /></div>
                                <p className="text-slate-400 text-sm font-medium">אין רכיבות להצגה</p>
                            </div>
                        )}
                            ...
                            >
                                <div className="flex items-center gap-3.5 rounded-2xl border border-white/5 bg-white/5 p-3 hover:border-white/10 hover:bg-white/10 backdrop-blur-sm transition-all shadow-sm">
                                    ...
                                    <div className="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-gradient-to-br from-slate-800 via-slate-700 to-emerald-900/40 ring-1 ring-white/10">
                                        {ride.imageUrl ? (
                                            <img src={imgSrc(ride.imageUrl)} alt="" className="h-full w-full object-cover" />
                                        ) : (
                                            <div className="flex h-full w-full items-center justify-center text-slate-600 text-xs">מפה</div>
                                        )}
                                    </div>
                                    {/* info */}
                                    <div className="flex-1 min-w-0 py-0.5">
                                        <p className="truncate font-semibold text-slate-100 text-sm">{ride.title}</p>
                                        ...
                                        <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-[11px] font-medium text-emerald-300">
                                            <span className="flex items-center gap-1 bg-emerald-500/10 px-2 py-0.5 rounded-md"><Timer className="w-3.5 h-3.5 text-emerald-400" /> {ride.duration}</span>
                                            <span className="flex items-center gap-1 bg-emerald-500/10 px-2 py-0.5 rounded-md"><MapPin className="w-3.5 h-3.5 text-emerald-400" /> {ride.distance}</span>
                                        </div>
                                    </div>
                ...
                {activeTab === "routes" && (
                    <div className="px-4 mt-6 space-y-3">
                        ...
                        {isRoutesLoading && (
                            <p className="text-center text-xs text-slate-400 py-6">טוען מסלולים...</p>
                        )}
                        {routesLoadError && !isRoutesLoading && (
                            <p className="text-center text-xs text-rose-300 py-6">{routesLoadError}</p>
                        )}

                        {/* empty state */}
                        {!isRoutesLoading && visibleRoutes.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-16 text-center opacity-80">
                                <div className="mb-4 flex justify-center"><MapIcon className="w-12 h-12 text-slate-500 opacity-50" /></div>
                                <p className="text-slate-400 text-sm font-medium">אין מסלולים שמורים</p>
                                <button
                                    type="button"
                                    onClick={() => setShowAddRoute(true)}
                                ...
                                ><Plus className="w-4 h-4" /> הוסף מסלול ראשון</button>
                            </div>
                        )}
                        ...
                        {visibleRoutes.map((route) => (
                            ...
                            >
                                <div className="flex items-center gap-3.5 rounded-2xl border border-white/5 bg-white/5 p-3 hover:border-white/10 hover:bg-white/10 backdrop-blur-sm transition-all shadow-sm">
                                    ...
                                    <div className="flex-1 min-w-0 py-0.5">
                                        <p className="truncate font-semibold text-slate-100 text-sm">{route.title}</p>
                                        <p className="text-xs text-slate-400 mt-1 truncate">
                                            {route.originLabel?.split(",")[0] || "מוצא לא ידוע"} ➝ {route.destinationLabel?.split(",")[0] || "יעד לא ידוע"}
                                        </p>
                                    </div>
                                    <span className="text-slate-500 ml-1">›</span>
                                </div>
            ...
            {showAddRoute && (
                <AddRouteModal
                    ...
                    newRouteTitle={newRouteTitle} setNewRouteTitle={setNewRouteTitle}
                    newOriginLabel={newOriginLabel} setNewOriginLabel={setNewOriginLabel}
                    newOriginLatLng={newOriginLatLng} setNewOriginLatLng={setNewOriginLatLng}
                    newDestinationLabel={newDestinationLabel} setNewDestinationLabel={setNewDestinationLabel}
                    newDestinationLatLng={newDestinationLatLng} setNewDestinationLatLng={setNewDestinationLatLng}
                    newRouteType={newRouteType} setNewRouteType={setNewRouteType}
                    newRouteDifficulty={newRouteDifficulty} setNewRouteDifficulty={setNewRouteDifficulty}
                    newRouteIsTwisty={newRouteIsTwisty} setNewRouteIsTwisty={setNewRouteIsTwisty}
                    activeMapPickField={activeMapPickField} setActiveMapPickField={setActiveMapPickField}
                    mapPickCenter={mapPickCenter} setMapPickCenter={setMapPickCenter}
                    mapPickStatus={mapPickStatus} setMapPickStatus={setMapPickStatus}
                    mapPickRequestIdRef={mapPickRequestIdRef}
                    isMapLoaded={isMapLoaded} mapLoadError={mapLoadError} mapApiKey={mapApiKey}
                    isValidMapPoint={isValidMapPoint}
                    originSuggestions={originSuggestions} setOriginSuggestions={setOriginSuggestions}
                    destinationSuggestions={destinationSuggestions} setDestinationSuggestions={setDestinationSuggestions}
                    activeSuggestionField={activeSuggestionField} setActiveSuggestionField={setActiveSuggestionField}
                    newRouteLocationError={newRouteLocationError} setNewRouteLocationError={setNewRouteLocationError}
                    isPlacesSuggestionsReady={isPlacesSuggestionsReady}
                    apiClient={apiClient} authToken={authToken} fetchRoutesFromServer={fetchRoutesFromServer}
