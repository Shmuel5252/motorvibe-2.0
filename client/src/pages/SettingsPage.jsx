...
export function AuthScreen({
  ...
  const handleSubmit = () => {
    if (
      !authEmail.trim() ||
      !authPassword.trim() ||
      (authMode === "register" && (!authName.trim() || !confirmPassword.trim()))
    ) {
  ...
  return (
    <div
      dir="rtl"
      className="mv-bg flex min-h-screen w-full flex-col font-sans text-slate-100"
    >
      <div className="mx-auto flex w-full max-w-6xl flex-col px-4 pb-10 pt-5 sm:px-6 flex-1">
        <main className="mt-6 flex-1 w-full flex flex-col items-center">
          ...
          <section className="text-center w-full max-w-md">
            <h1 className="text-3xl font-bold leading-tight sm:text-4xl bg-linear-to-br from-white to-gray-400 bg-clip-text text-transparent">
              {authMode === "register" ? "יצירת חשבון" : "ברוכים השבים"}
            </h1>
            <p className="mt-2 text-base text-gray-400 sm:text-lg">
              {authMode === "register"
                ? "הצטרפו לחוויית רכיבה מתקדמת"
                : "התחברו כדי לנהל מסלולים שמורים"}
            </p>
          </section>
          ...
          <section className="mt-8 w-full max-w-md">
              ...
              <div className="space-y-3 relative z-10 w-full">
                ...
                {authMode === "register" && (
                  <div className="grid grid-cols-2 gap-x-2 gap-y-2 mt-2 mb-1 text-[11px] sm:text-xs">
                    <div
                      className={`flex items-center gap-1.5 transition-colors duration-300 ${pwdRules.length ? "text-emerald-400" : "text-gray-400"}`}
                    >
                      {pwdRules.length ? (
                    ...
                    <div
                      className={`flex items-center gap-1.5 transition-colors duration-300 ${pwdRules.uppercase ? "text-emerald-400" : "text-gray-400"}`}
                    >
                      {pwdRules.uppercase ? (
                        <CheckCircle2 size={14} />
                      ) : (
                    ...
                    <div
                      className={`flex items-center gap-1.5 transition-colors duration-300 ${pwdRules.lowercase ? "text-emerald-400" : "text-gray-400"}`}
                    >
                      {pwdRules.lowercase ? (
                        <CheckCircle2 size={14} />
                      ) : (
                    ...
                    <div
                      className={`flex items-center gap-1.5 transition-colors duration-300 ${pwdRules.symbol ? "text-emerald-400" : "text-gray-400"}`}
                    >
                      {pwdRules.symbol ? (
                        <CheckCircle2 size={14} />
                      ) : (
            ...
            <div className="mt-6 flex flex-col items-center gap-4 pb-4 border-b border-transparent">
              ...
              <button
                type="button"
                onClick={() => {
                  window.location.href = `${import.meta.env.VITE_API_BASE_URL?.replace("/api", "") || "http://localhost:5000"}/api/auth/google`;
                }}
