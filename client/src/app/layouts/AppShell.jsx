...
function AppShell({ children, onLogout, isAuthenticated = false }) {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => setRideElapsedSeconds((p) => p + 1), 1000);
  ...
  const handleRideActiveChange = (isActive) => {
    ...
    if (isActive) { setIsDrawerOpen(false); return; }
    ...
    <div dir="rtl" className="mv-bg">
      ...
      {isRideActive && isRideMinimized && (
        <button
          ...
          onClick={() => { setIsRideMinimized(false); onNavigate("ride"); }}
        >
          <span className="font-mono text-base font-semibold tracking-wide">{minutes}:{seconds}</span>
          <span className={["mv-pill px-2.5 py-1 text-xs", isRidePaused ? "text-amber-200" : "text-emerald-200"].join(" ")}>
